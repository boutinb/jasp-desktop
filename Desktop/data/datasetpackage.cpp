...
int DataSetPackage::columnCount(const QModelIndex &parent) const
{
	...
	case dataSetBaseNodeType::dataSet:
	{
		Log::log() << "COLUMNCOUNT DataSetPackage dataSet: 1" << std::endl;
	...
	case dataSetBaseNodeType::data:
	{
		...
		int c = data->columnCount();
		Log::log() << "COLUMNCOUNT DataSetPackage data: " << c << std::endl;
		return c;
	}
	...
	case dataSetBaseNodeType::filters:
	{
		//DataSet * data = dynamic_cast<DataSet*>(node->parent());
		//return data->rowCount();
		Log::log() << "COLUMNCOUNT DataSetPackage filters: 1" << std::endl;
	...
	case dataSetBaseNodeType::column:
	{
		...
		int labelSize = col->labels().size();
		Log::log() << "COLUMNCOUNT DataSetPackage column: " << labelSize << std::endl;
		return labelSize;
	}
		
	case dataSetBaseNodeType::filter:
	case dataSetBaseNodeType::label:
		Log::log() << "COLUMNCOUNT DataSetPackage filter or label: 1" << std::endl;

		return 1;
	
	case dataSetBaseNodeType::unknown:
		Log::log() << "COLUMNCOUNT DataSetPackage unknown: 0" << std::endl;
		return 0;
	}

	Log::log() << "COLUMNCOUNT DataSetPackage !!!: 0" << std::endl;
	return 0; // <- because gcc is stupid
}
