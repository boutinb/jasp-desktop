		...
		JASPDataView
		{
			...
			function showCopyPasteMenu(fromItem, globalPos, rowIndex, columnIndex)
			{
				...
				[
					...
					{ text: qsTr("Header cut"),	shortcut: qsTr("%1+Shift+X").arg(ctrlCmd),	func: function() { dataTableView.view.cut(true) }   ,						icon: "menu-data-paste"				},
					{ text: qsTr("Header copy"),shortcut: qsTr("%1+Shift+C").arg(ctrlCmd),	func: function() { dataTableView.view.copy(true) }  ,						icon: "menu-data-copy"				},
					{ text: qsTr("Header paste"), shortcut: qsTr("%1+Shift+V").arg(ctrlCmd),func: function() { dataTableView.view.paste(true) } ,						icon: "menu-data-cut"				},
