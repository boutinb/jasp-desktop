#ifndef DUP_JASPCONTROL_H
#define DUP_JASPCONTROL_H
...
class DUP_JASPControl : public QQuickItem
{
	Q_OBJECT
	...
	Q_PROPERTY( DUP_JASPControl						*	parentListView			READ parentListViewEx										NOTIFY parentListViewChanged		)
	...
	typedef std::set<DUP_JASPControl*> Set;
...
public:
	...
	DUP_JASPControl(QQuickItem *parent = nullptr);
	...
	DUP_AnalysisForm	*	form()						const	{ return _form;					}
	QQuickItem		*	childControlsArea()			const	{ return _childControlsArea;	}
	DUP_JASPListControl	*	parentListView()			const	{ return _parentListView;		}
	DUP_JASPControl		*	parentListViewEx()			const;
	...
	QVector<DUP_JASPControl::ParentKey>	getParentKeys();

	QQmlComponent				*	rowComponent()						const { return _rowComponent;	}

	static QString					ControlTypeToFriendlyString(ControlType controlType);
	static QList<DUP_JASPControl*>		getChildJASPControls(const QQuickItem* item);

	virtual void					setUp()										{}
	virtual void					cleanUp()									{ disconnect(); }
	virtual DUP_BoundControl		*	boundControl();
	virtual bool					encodeValue()						const	{ return false; }

	const Set					&	depends()							const	{ return _depends; }
	bool							addDependency(DUP_JASPControl* item);
	void							removeDependency(DUP_JASPControl* item);
	virtual DUP_JASPControl			*	getChildControl(QString key, QString name)	{ return nullptr; }
...
signals:
	...
	void boundValueChanged(DUP_JASPControl* control);
...
protected:
	...
	DUP_AnalysisForm*			_form						= nullptr;
	QString					_name,
							_info,
							_toolTip,
							_title,
	...
	DUP_JASPListControl		*	_parentListView				= nullptr;
