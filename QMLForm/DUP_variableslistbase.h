...
#ifndef DUP_VARIABLESLISTBASE_H
#define DUP_VARIABLESLISTBASE_H
...
class DUP_ListModelDraggable;
class DUP_CheckBoxBase;

class DUP_VariablesListBase : public DUP_JASPListControl, public DUP_BoundControl
{
	Q_OBJECT
...
public:
	DUP_VariablesListBase(QQuickItem* parent = nullptr);
	
	void						setUp()										override;
	DUP_ListModel*					model()								const	override;
	DUP_ListModelDraggable*			draggableModel()					const				{ return _draggableModel;							}
	...
	DUP_BoundControl*				boundControl()								override	{ return _boundControl;								}
	...
	bool						addRowControl(const QString& key, DUP_JASPControl* control) override;
	void						moveItems(QList<int> &indexes, DUP_ListModelDraggable* dropModel, int dropItemIndex = -1);
...
protected:
	...
	DUP_ListModel*					getRelatedModel();

	DUP_ListModelDraggable*			_draggableModel	= nullptr;
	ListViewType				_listViewType	= ListViewType::AssignedVariables;
	DUP_BoundControl*				_boundControl	= nullptr;
...
protected slots:
	...
	void interactionHighOrderHandler(DUP_JASPControl* checkBoxControl);
...
private:
	...
	DUP_ListModelDraggable	*		_tempDropModel = nullptr;
