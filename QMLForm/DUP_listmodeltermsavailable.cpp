...
void DUP_ListModelTermsAvailable::resetTermsFromSources(bool updateAssigned)
{
	
	beginResetModel();

	DUP_Terms termsAvailable = getSourceTerms();
	DUP_Terms removedTerms, addedTerms;
	
	for (const DUP_Term& term : _allTerms)
		if (!termsAvailable.contains(term))
			removedTerms.add(term);

	for (const DUP_Term& term : termsAvailable)
