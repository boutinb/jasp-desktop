...
convertAbsoluteSymlinksToRelative <- function(modulesRoot, renvCache, prefixTarget)
{
  ...
  else
  {
    ...
    for(row in seq(nrow(symlinks)))
    {
      ...
      linkTarget = paste0(prefixTarget, symlinks[row, "linkTarget"])
      print(paste0("For link '", padToMax(symlinks[row, "linkLocation"], 1), "' will convert '", padToMax(symlinks[row, "originalTarget"], 2), "' to '", padToMax(linkTarget, 3), "'"))
      file.symlink(from=linkTarget, to=basename(linkLoc))
    }
  }
}
